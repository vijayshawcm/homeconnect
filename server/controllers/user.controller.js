	...
	try {
		...
		if (verifyToken) {
			...
			const user = await User.findOne({
				'userInfo.username': decodedToken.username,
			}); // Query user
			...
			const response = {
				username: user.userInfo.username,
				fullName: user.userInfo.displayName,
				email: user.userInfo.email,
				bio: user.userInfo.aboutMe,
...
const registerUser = async (req, res) => {
	if (
		(await User.findOne({ 'userInfo.email': req.body.email })) ||
		(await User.findOne({ 'userInfo.username': req.body.username }))
	) {
		return res
			.status(409)
			.json('Account or Email has already been registered!'); // Woah 409 code!
	}
